...
class RoleResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->actions([
                ViewAction::make()
                    ->hidden(fn (Role $role) => self::canEdit($role)),
                EditAction::make(),
                DeleteAction::make()
                    ->hidden(fn (Role $role) => $role->isRootAdmin() || $role->users()->count() > 0),
            ])
            ->checkIfRecordIsSelectableUsing(fn (Role $role) => !$role->isRootAdmin() && $role->users()->count() <= 0)
            ->bulkActions([
