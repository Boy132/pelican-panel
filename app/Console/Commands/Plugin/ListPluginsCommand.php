...
class ListPluginsCommand extends Command
{
    ...
    {
        $plugins = Plugin::query()->get(['package', 'class', 'status', 'name', 'author', 'panel', 'category']);

        if ($this->option('format') === 'json') {
            $this->output->write($plugins->toJson(JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
        } else {
            $this->table(['Package Name', 'Main Class', 'Status', 'Name', 'Author', 'Panel', 'Category'], $plugins->toArray());
        }
